...
pub fn read_games_from_file (filestr: String, games: &mut Vec<String>) {
    ...
    let f = match fs::File::open(&filestr) {
        Ok(fich) => fich,
        Err(_) => panic!("no se puede abrir fichero"),
    };

    let fich = BufReader::new(&f);

    // first an empty game for convenience
    arr_parti_txt.push(globs::EMPTY_GAME.to_string());
    ...
    while i < len {
        ...
        if moves[i] == "{" {
            ...
            if starts_with_comment {    // == true 
                ...
                // this does not work with adecuate margins -- TODO
            } else {
        ...
        if moves[i] == "--" {
            ...
            nodes.set_san(cur_indx.clone(), "--".to_string());
            comment_indx = cur_indx.clone();
            let tmp_fen = nodes.get_fen(parent_indx.clone());
            let nu_mov = null_move(tmp_fen);
            nodes.set_fen(cur_indx.clone(), nu_mov);
        ...
        if "abcdefghRBNQKO0".contains(&moves[i].to_string()[0..1])  {
            ...
            if mov.movim.move_int == 0 {    // move is error
                let mut nodeserror = globs::ListNodes::new();
                // retornamos una partida vacia - we return an empty game
                nodeserror.init_node (lh::root_node());
                nodeserror.init_branch_level(lh::root_node(), 0);
                nodeserror.init_fen(lh::root_node(), globs::DEFAULT_POSITION.to_string());
                nodeserror.set_comment(lh::root_node(), "error in game moves".to_string());
                return nodeserror;
            }
        ...
        if moves[i] == ")" {
            starts_with_comment = true;
			// a safeguard
            if branch_level == 0 { // no parent (some error are in game pgn moves)
                let mut nodeserror = globs::ListNodes::new();
                // retornamos una partida vacia
                nodeserror.init_node (lh::root_node());
                nodeserror.init_branch_level(lh::root_node(), 0);
                nodeserror.init_fen(lh::root_node(), globs::DEFAULT_POSITION.to_string());
                nodeserror.set_comment(lh::root_node(), "error in game moves".to_string());
...
fn null_move(fen: String) -> String {
    
    let old_fen: Vec<&str> = fen.split(" ").collect();
    let old_pos = old_fen[0];
    let old_color = old_fen[1];
    let old_castling = old_fen[2];
    //let old_en_passant = old_fen[3];
    let old_halfmove: i32 = old_fen[4].to_string().parse::<i32>().unwrap();
    let old_fullmove = old_fen[5].to_string().parse::<i32>().unwrap();
    ...
    let new_en_passant = "-";
    let new_halfmove = old_halfmove + 1;
    let new_fullmove: i32;
    if old_color == "b" {
        new_fullmove = old_fullmove +1;
    }
    else {
        new_fullmove = old_fullmove;
    }
    
    let ffen = format!("{} {} {} {} {} {}", 
            new_pos, new_color, new_castling, 
            new_en_passant, new_halfmove, new_fullmove);
    ffen
}
...
pub fn nodes_to_buffer(view: &gtk::TextView, nodes: &mut globs::ListNodes, node_indx: String) {
    ...
    if nodes.node_exists(parent_indx.clone()) &&
        ...
        if branch_level < GTK_INDENT_LEVEL {
            ...
            if nodes.get_start_comment(node_indx.clone()).len() > 0 {
                let comm:String = format!("{} ", nodes.get_start_comment(node_indx.clone()));
            ...
            buf.apply_tag_by_name("comment", &start_iter, &end_iter);
        }
        else {
    ...
    if display_mv_nr || node_indx == lh::get_next_mainline_indx(lh::root_node()) {
        if side_to_move == "b".to_string() {
            ...
            let nr = format!("{}.", mv_nr.parse::<i32>().unwrap());
            let mut start_iter = buf.get_end_iter();
    ...
    if nodes.nodes[&node_indx.clone()].san.len() > 0 {
        ...
        let level = get_tag_branchlevel(branch_level.clone());
        buf.apply_tag_by_name(level.as_str(), &start_iter, &end_iter);
        buf.apply_tag_by_name("figurine", &start_iter, &end_iter);
