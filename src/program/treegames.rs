...
impl TreeGames {
    ...
    pub fn closures_treegame (self, 
        ...
        self.tview.clone().connect_row_activated(move |widget, path, _col| {
            let modelo = widget.get_model().unwrap();
            let iter = modelo.get_iter(path).unwrap();
