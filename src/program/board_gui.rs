...
impl BoardGui {
    pub fn new() -> Self {
        ...
        area_tablero.connect_draw ( move |widget, ctx| {
            ...
            //pango::LayoutExt::set_font_description(&layout, &desc);
            ...
            */
            for j in 0..8 {
                ...
                let coord;
                {
                    let h: u8 = 104;  // ascii values
                    let a: u8 = 97;
                    if volteado {
                        coord = ((h - j) as char).to_string();
                    }
                    else {
                        coord = ((a + j) as char).to_string();
                    }
                }
                //pango::LayoutExt::set_text(&layout, &coord);
                layout.set_text(&coord);
                //let (pix_ancho, pix_alto) = pango::LayoutExt::get_pixel_size(&layout);  // ->  (i32, i32)
                let (_pix_ancho, pix_alto) = layout.get_pixel_size();
                cairo::Context::translate(ctx, (j as f64 * tx) + padding_coord, height as f64 - pix_alto as f64 - padding_coord); // (&self, tx: f64, ty: f64)
            ...
            for j in 0..8 {
                ...
                let coord;
                {
                    let uno: u8 = 49;  // ascii values
                    let ocho: u8 = 56;
                    if volteado {
                        coord = ((uno + j) as char).to_string();
                    }
                    else {
                ...
                //pango::LayoutExt::set_text(&layout, &coord);
                layout.set_text(&coord);
                //let (pix_ancho, pix_alto) = pango::LayoutExt::get_pixel_size(&layout);
                let (pix_ancho, _pix_alto) = layout.get_pixel_size();
                cairo::Context::translate(ctx, width as f64 - pix_ancho as f64, (j as f64 * tx) + padding_coord);
                pangocairo::functions::show_layout(ctx, &layout);
                cairo::Context::restore(ctx);
                light = !light;
            }
